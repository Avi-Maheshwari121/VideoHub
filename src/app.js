//this is a express app that will be exported to index.js.....
...
app.use(
  cors({
    origin: process.env.CORS_ORIGIN,  //origin we are allowing to communicate with our backend.
    credentials: true,
  })
);
...
app.use(express.urlencoded({ extended: true, limit: "20kb" })); // a middleware in Express.This middleware is used to parse incoming requests with application/x-www-form-urlencoded payloads. It is typically used to handle data submitted via HTML forms(URL). The parsed data is then accessible in req.body. like in some URL + is used while in some %20 is used etc.. the extended: true helps to parse nested objects.
